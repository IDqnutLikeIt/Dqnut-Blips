...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('Dqnut-Blips:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('Dqnut-Blips:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        ESX.ShowNotification('Blips activated.', 'success')
    else
        ShowBlips = false
        ESX.ShowNotification('Blips deactivated.', 'error')
    end
end)

RegisterNetEvent('Dqnut-Blips:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        ESX.ShowNotification('Nammes activated.', 'success')
    else
        ShowNames = false
        ESX.ShowNotification('Names deactivated.', 'error')
    end
end)

RegisterNetEvent('Dqnut-Blips:client:Show', function(players)
